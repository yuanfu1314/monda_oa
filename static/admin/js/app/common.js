    ...
    define(function(require, exports) {
		...
		* 返回查找值
		*/
        exports.searchData = function (search_obj) {
			var search_data = {};

			$.each(search_obj, function(index, el) {
				var data = $(el).val();
				if (null !== data) {
					data = $.trim(data);	
				} 	
				if ('' != data) {
					search_data[$(el).attr('data-id')] = data;
				} 
			});
		...
		/**
		* 返回查询参数
		* 将json参数转为字符串,避免传送数据参数过大
		*/
		exports.getSearchData = function () {
			return exports.searchData($(".search input, .search select"));
		}
		//无刷新查询
